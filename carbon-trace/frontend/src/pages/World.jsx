...
export default function World() {
  ...
  // 测一下 WS 通道通不通（Phase 3 前是 echo stub）
  useEffect(() => {
    const proto = location.protocol === 'https:' ? 'wss' : 'ws'
    const ws = new WebSocket(`${proto}://${location.host}/ws/world`)
    ws.onopen = () => setWsStatus('已连接（stub）')
    ws.onmessage = (e) => {
      try {
        const msg = JSON.parse(e.data)
        if (msg.type === 'stub') setWsStatus(`stub: ${msg.message}`)
      } catch { /* ignore */ }
    }
    ws.onclose = () => setWsStatus('连接断开')
    ws.onerror = () => setWsStatus('连接失败（后端未启动？）')
    return () => ws.close()
  }, [])
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pt-5 space-y-3">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-ink-900 tracking-tight">碳迹世界</h1>
